<!DOCTYPE html>
<html>
	<?php 
	$pageName="itinerary-budapest-vienna-prague";
	$tagTitle="行程 - EF TravelPlus | 英孚旅游+";
	$metaTitle="欧洲旅游新选择_EF英孚旅游_一站式旅游专家";
	$metaKeywords="欧洲旅游，EF英孚旅游，EFTravelPlus，品质跟团游，EF英孚教育，告别走马观花，体验式旅行";
	$metaDescription="英孚旅游+,是英孚旗下一站式旅游服务专家，在全球众多国家拥有专业的旅游执行团队和完善的网络体系，用有趣的方式，让你和有趣的年轻人一起，发现未知的精彩世界。";

	$itineraryName="东欧捷奥匈帝国之旅";
	$itineraryPrice="￥16,400";

    include_once("itinerary-head.php"); 
?>

	<body data-spy="scroll" data-target="#travelPlusNav">
		<?php include_once('fix-cta.php'); echo $text1 ;?>
		<?php include_once('common-header.php') ;?>
		<!-- Image -->
		<div class="imagesWrap" style="background:url(img/itinerary/billboard/budapest-vienna-prague.jpg)  center center no-repeat;
				background-size:cover;">
			<div class="site">
				<div class="tourInfo">
					<div class="transparentBg"></div>
					<div class="transparentTxt">
						<div class="yellowTxt">圣诞 / 元旦 / 春节出发</div>
						<div class="tourName">东欧捷奥匈帝国之旅</div>
						<div class="flex-row-start-between">
							<div class="rate">
								<i class="fa fa-star active" aria-hidden="true"></i>
								<i class="fa fa-star active" aria-hidden="true"></i>
								<i class="fa fa-star active" aria-hidden="true"></i>
								<i class="fa fa-star active" aria-hidden="true"></i>
								<i class="fa fa-star active" aria-hidden="true"></i>
							</div>
							<div class="pinkTxt">2人同行，立减4000</div>
						</div>
					</div>
				</div>
				<?php include_once('discount.php'); echo $discount2 ;?>
			</div>
		</div>
		<!-- Itinerary Info -->
		<div class="site">
			<div class="itineraryInfo">
				<div class="flex-row-start-around itineraryBasicInfo">
					<div class="flex1 itineraryLine">
						<div class="days">11天</div>
						<div class="flex-row-start-between">
							<div>布达佩斯</div>
							<i class="grayArrow"></i>
							<div>维也纳</div>
							<i class="grayArrow"></i>
							<div>布拉格</div>
							<i class="grayArrow"></i>
						</div>
					</div>
					<div class="flex1 flex-row-start-between">
						<div class="priceWG">
							￥16,400
							<span>起</span>
						</div>
						<div>
							<a class="blueRBtn" href="#getPersonalizedPrice">获取实时价格</a>
						</div>
					</div>
				</div>
				<div class="flex-row-start-around itineraryHighLights">
					<div class="flex1 tourGuide">
						<p class="serif">当你因《布达佩斯大饭店》、金色大厅和布拉格广场来到这里，你会发现热情的东欧人带给你的远不止这些，你会深深迷恋上多瑙河和伏尔塔瓦河畔的这些城市，终有一天完美的旅程回忆会把你再次带回到这里。</p>
						<div class="com_profile">
							<div class="imgWrap">
								<img src="img/itinerary/tourDirector/Constantin.jpg" />
							</div>
							<div class="userInfo">
								<span class="position">外籍导游</span>
								<span class="name">Constantin</span>
							</div>
						</div>
					</div>
					<div class="flex1 hightLight">
						<h4>行程亮点</h4>
						<p>
							与当地导游一起领略中东欧热情的异域文化<br/> 
							在布达佩斯城堡山俯瞰多瑙河贯穿整个城市<br/> 
							在维也纳与当地人一起共进晚餐<br/> 
							进入茜茜公主的宫殿参观<br/> 
							品尝正宗的捷克啤酒<br/> 
							乘坐布拉格有轨电车<br/>
							充足的自由行时间可自行定制旅行
						</p>
					</div>
				</div>
			</div>
		</div>
		<!-- Swiper -->
		<div class="site itineraryPhoto">
			<div class="swiper-container">
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/itinerary/budapest-vienna-prague01.jpg" />
							</div>
							<div class="bgIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">梵蒂冈不可思议的建筑</div>
							</div>
						</div>
						<div class="photoBy">
							<div class="com_profile">
								<div class="imgWrap">
									<img src="img/itinerary/tourDirector/Gattazzo.jpg" />
								</div>
							</div>
							Photo by Anita
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/itinerary/budapest-vienna-prague02.jpg" />
							</div>
							<div class="bgIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">梵蒂冈不可思议的建筑</div>
							</div>
						</div>
						<div class="photoBy">
							<div class="com_profile">
								<div class="imgWrap">
									<img src="img/itinerary/tourDirector/Gattazzo.jpg" />
								</div>
							</div>
							Photo by Anita
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/itinerary/budapest-vienna-prague03.jpg" />
							</div>
							<div class="bgIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">梵蒂冈不可思议的建筑</div>
							</div>
						</div>
						<div class="photoBy">
							<div class="com_profile">
								<div class="imgWrap">
									<img src="img/itinerary/tourDirector/Gattazzo.jpg" />
								</div>
							</div>
							Photo by Anita
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/itinerary/budapest-vienna-prague04.jpg" />
							</div>
							<div class="bgIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">梵蒂冈不可思议的建筑</div>
							</div>
						</div>
						<div class="photoBy">
							<div class="com_profile">
								<div class="imgWrap">
									<img src="img/itinerary/tourDirector/Gattazzo.jpg" />
								</div>
							</div>
							Photo by Anita
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/itinerary/budapest-vienna-prague05.jpg" />
							</div>
							<div class="bgIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">梵蒂冈不可思议的建筑</div>
							</div>
						</div>
						<div class="photoBy">
							<div class="com_profile">
								<div class="imgWrap">
									<img src="img/itinerary/tourDirector/Gattazzo.jpg" />
								</div>
							</div>
							Photo by Anita
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/itinerary/budapest-vienna-prague06.jpg" />
							</div>
							<div class="bgIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">梵蒂冈不可思议的建筑</div>
							</div>
						</div>
						<div class="photoBy">
							<div class="com_profile">
								<div class="imgWrap">
									<img src="img/itinerary/tourDirector/Gattazzo.jpg" />
								</div>
							</div>
							Photo by Anita
						</div>
					</div>
				</div>
				<!--<div class="swiper-button-prev"></div>
    				<div class="swiper-button-next"></div>-->
				<!-- 如果需要分页器 -->
				<div class="swiper-pagination">
				</div>
			</div>
		</div>
		<!-- Include -->
		<div class="section blueBg">
			<div class="site">
				<h4>价格包含</h4>
				<div class="includedItems flex-row-start-start section">
					<div class="">
						<div class="imgWrap">
							<img src="img/page-1.svg" />
						</div>
						来往机票
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-2.svg" />
						</div>
						9晚精选酒店住宿
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-3.svg" />
						</div>
						每日早餐
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-4.svg" />
						</div>
						3顿三道式晚餐,供应啤酒或葡萄酒
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-5.svg" />
						</div>
						多语言外籍导游和中方领队
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-6.svg" />
						</div>
						专用豪华大巴接送
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-7.svg" />
						</div>
						陪同观光和精选景点门票
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-8.svg" />
						</div>
						除自由时间外涉及的海外服务小费
					</div>
					<div class="">
						<div class="imgWrap">
							<img src="img/page-9.svg" />
						</div>
						旅行期间的保险
					</div>
				</div>
			</div>
		</div>
		<!-- Itinerary -->
		<div class="site">
			<h4>行程安排</h4>
			<div class="itineraryIntro section">
				<div class="initialScreenLinear"></div>
				<div class="itineraryMap">
					<img src="img/itinerary/map/budapest-vienna-prague.jpg" />
				</div>
				<div class="initialScreen">
					<div class="com_itinerary_summary">
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第1天</label><span>出发</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>启程并抵达布达佩斯</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第2天</label><span>布达佩斯观光游</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>									
									<li>坐巴士前往英雄广场</li>
									<li>进入布达佩斯标志性建筑国会大厦参观</li>
									<li>抵达多瑙河左岸的布达城，登上城堡山俯瞰布达佩斯全景</li>
									<li>参观著名的哥特式建筑马加什教堂</li>
									<li>在迎接晚宴上结识同行伙伴</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第3天</label><span>布达佩斯自由行</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>布达佩斯自由行</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第4天</label><span>前往圣安德烈和维也纳</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>上午乘大巴前往匈牙利小镇圣安德烈自由行动</li>
									<li>午饭过后前往维也纳，途径维谢格拉德城堡和埃斯泰尔戈姆圣殿</li>
									<li>和维也纳当地人家共进晚餐</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第5天</label><span>维也纳观光游</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>导游带领参观美泉宫</li>
									<li>乘坐巴士领略维也纳风光，途经维也纳国家歌剧院、议会大厦、霍夫堡皇宫和百水公寓</li>
									<li>到维也纳市中心的“维也纳心脏”圣史蒂芬大教堂合影留念</li>
									<li>下午自由活动</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第6天</label><span>维也纳自由行</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>维也纳自由行</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第7天</label><span>前往布拉格，下午自由活动</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>前往布拉格</li>
									<li>坐邮轮夜游伏尔塔瓦河，欣赏布拉格如画夜景</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第8天</label><span>布拉格观光游</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>登上布拉格城堡山，俯瞰砖红色的布拉格建筑群</li>
									<li>进入布拉格城堡区的重要地标建筑圣维特大教堂参观</li>
									<li>坐上有轨电车下山前往伏尔塔瓦河</li>
									<li>步行穿过查理大桥，观看桥上各式各样的现场表演</li>
									<li>前往老城广场参观布拉格天文钟和泰恩教堂</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第9天</label><span>布拉格自由行</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>布拉格自由行</li>
									<li>晚上在布拉格享受告别晚宴</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第10天</label><span>返程</span>
							</div>
							<div class="itinerary_daily_detail">
								<ul>
									<li>前往机场搭乘返航航班</li>
								</ul>
							</div>
						</div>
						<div class="itinerary_daily_item">
							<div class="itinerary_daily_title">
								<div class="itemCircle"></div>
								<label>第11天</label><span>抵达中国</span>
							</div>
						</div>
						<p>请注意，以上为示范行程，行程内容可能会因出发日期、航班时间或其他因素发生变动，详情请拨打电话 400-180-7518 查询。</p>
					</div>
				</div>
			</div>
			<button class="borderBtn allItinerary">查看完整行程</button>
		</div>

		<!-- Hotels -->
		<div class="site hotels">
			<h4>精选酒店</h4>
			<p class="notes">以下是我们的备选酒店，入住视具体情况而定。</p>
			<div class="swiper-container">
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/hotel/Radisson Blu Beke Hotel/Radisson Blu Beke Hotel_ext.jpg" />
							</div>
							<div class="hotelIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">Radisson Blu Beke Hotel 布达佩斯贝克丽笙布鲁酒店</div>
							</div>
						</div>
						<div class="hotelDes">Radisson Blu Béke酒店装饰典雅，提供免费室内游泳池、桑拿浴室和覆盖各处的免费WiFi。意式美食餐厅供应地中海菜肴和匈牙利美食。咖啡厅提供三明治、沙拉和自制糕点。酒店设有内部礼品店，亦可应要求提供会议设施。</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/hotel/Radisson Blu Beke Hotel/Radisson Blu Beke Hotel_room.jpg" />
							</div>
							<div class="hotelIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">Radisson Blu Beke Hotel 布达佩斯贝克丽笙布鲁酒店</div>
							</div>
						</div>
						<div class="hotelDes">Radisson Blu Béke酒店装饰典雅，提供免费室内游泳池、桑拿浴室和覆盖各处的免费WiFi。意式美食餐厅供应地中海菜肴和匈牙利美食。咖啡厅提供三明治、沙拉和自制糕点。酒店设有内部礼品店，亦可应要求提供会议设施。</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/hotel/Marriott Vienna Schoenbrunn/Marriott Vienna Schoenbrunn_room.jpg" />
							</div>
							<div class="hotelIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">Courtyard by Marriott Vienna Schoenbrunn 维也纳美泉宫万怡酒店</div>
							</div>
						</div>
						<div class="hotelDes">这家四星级酒店毗邻著名的美泉宫和美泉宫动物园，Courtyard by Marriott Wien Schönbrunn酒店的现代化空调客房均配备了平面电视、吹风机、办公桌以及沏茶/咖啡设备。酒店公共区域提供免费无线网络连接。酒店的La Corte餐厅供应意大利和奥地利美食。客人可以在大堂酒吧休闲放松，亦可在24小时营业的市场品尝小吃。</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/hotel/Marriott Vienna Schoenbrunn/Marriott Vienna Schoenbrunn_dinning.jpg" />
							</div>
							<div class="hotelIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">Courtyard by Marriott Vienna Schoenbrunn 维也纳美泉宫万怡酒店</div>
							</div>
						</div>
						<div class="hotelDes">这家四星级酒店毗邻著名的美泉宫和美泉宫动物园，Courtyard by Marriott Wien Schönbrunn酒店的现代化空调客房均配备了平面电视、吹风机、办公桌以及沏茶/咖啡设备。酒店公共区域提供免费无线网络连接。酒店的La Corte餐厅供应意大利和奥地利美食。客人可以在大堂酒吧休闲放松，亦可在24小时营业的市场品尝小吃。</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/hotel/Marriott Vienna Schoenbrunn/Marriott Vienna Schoenbrunn_gym.jpg" />
							</div>
							<div class="hotelIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">Courtyard by Marriott Vienna Schoenbrunn 维也纳美泉宫万怡酒店</div>
							</div>
						</div>
						<div class="hotelDes">这家四星级酒店毗邻著名的美泉宫和美泉宫动物园，Courtyard by Marriott Wien Schönbrunn酒店的现代化空调客房均配备了平面电视、吹风机、办公桌以及沏茶/咖啡设备。酒店公共区域提供免费无线网络连接。酒店的La Corte餐厅供应意大利和奥地利美食。客人可以在大堂酒吧休闲放松，亦可在24小时营业的市场品尝小吃。</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/hotel/NH Prague City/NH Prague City_dinning.jpg" />
							</div>
							<div class="hotelIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">NH Prague City 布拉格市NH酒店</div>
							</div>
						</div>
						<div class="hotelDes">NH Prague酒店位于历史悠久的市中心边缘现代化的Andel商务区，由缆车相连的2栋独立建筑组成，享有布拉格的全景。2栋建筑内均提供免费WiFi。客人可以在餐厅或俯瞰着整个布拉格市的露台上享用一流的佳肴。酒店自家的健康区亦提供桑拿浴和蒸汽浴设施、按摩理疗服务和健身中心。</div>
					</div>
					<div class="swiper-slide">
						<div class="swiperBgWrap">
							<div class="slideBg">
								<img src="img/hotel/NH Prague City/NH Prague City_gym.jpg" />
							</div>
							<div class="hotelIntro">
								<div class="transparentBg"></div>
								<div class="transparentTxt">NH Prague City 布拉格市NH酒店</div>
							</div>
						</div>
						<div class="hotelDes">NH Prague酒店位于历史悠久的市中心边缘现代化的Andel商务区，由缆车相连的2栋独立建筑组成，享有布拉格的全景。2栋建筑内均提供免费WiFi。客人可以在餐厅或俯瞰着整个布拉格市的露台上享用一流的佳肴。酒店自家的健康区亦提供桑拿浴和蒸汽浴设施、按摩理疗服务和健身中心。</div>
					</div>
				</div>
				<!-- 如果需要分页器 -->
				<div class="swiper-pagination">
				</div>
			</div>
		</div>

		<!-- Flight -->
		<div class="site flight">
			<h4 class="flex-row-end-between">
				航班信息
				<div>
					<label>出发城市:</label>
					<select>
						<option>北京</option>
						<option>上海</option>
						<option>广州</option>
						<option>香港</option>
						<option>成都</option>
					</select>
				</div>
			</h4>
			<p class="notes">以下是参考航班信息，以最终实际预订情况为准。</p>
			<div class="swiper-container section">
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						<div class="departure">成都出发</div>
						<div class="autoWrapper">
							<div class="com_flightInfo">
								<div class="voyage">启程</div>
								<div class="ibWrap">
									<div class="">荷兰皇家</div>
									<div class="">14h35m</div>
									<div class="">1次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>成都</span>15:30</div>
									<div class="airport"><span>阿姆斯特丹</span>19:15</div>
									<div class="airport"><span>阿姆斯特丹</span>21:05</div>
									<div class="airport"><span>布达佩斯</span>23:05</div>
								</div>
							</div>
							<div class="com_flightInfo">
								<div class="voyage">回程</div>
								<div class="ibWrap">
									<div class="">荷兰皇家、东方航空</div>
									<div class="">23h0m</div>
									<div class="">2次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>布拉格</span>14:30</div>
									<div class="airport"><span>阿姆斯特丹</span>16:00</div>
									<div class="airport"><span>阿姆斯特丹</span>17:15</div>
									<div class="airport"><span>上海</span>10:50+1</div>
									<div class="airport"><span>上海</span>17:00</div>
									<div class="airport"><span>成都</span>20:30</div>
								</div>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="departure">上海出发</div>
						<div class="autoWrapper">
							<div class="com_flightInfo">
								<div class="voyage">启程</div>
								<div class="ibWrap">
									<div class="">卡塔尔航空</div>
									<div class="">18h15m</div>
									<div class="">1次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>上海</span>23:15</div>
									<div class="airport"><span>多哈</span>04:55+1</div>
									<div class="airport"><span>多哈</span>06:45</div>
									<div class="airport"><span>布达佩斯</span>10:30</div>
								</div>
							</div>
							<div class="com_flightInfo">
								<div class="voyage">回程</div>
								<div class="ibWrap">
									<div class="">卡塔尔航空</div>
									<div class="">15h5m</div>
									<div class="">1次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>布拉格</span>16:10</div>
									<div class="airport"><span>多哈</span>00:15+1</div>
									<div class="airport"><span>多哈</span>01:05</div>
									<div class="airport"><span>上海</span>14:15</div>
								</div>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="departure">北京出发</div>
						<div class="autoWrapper">
							<div class="com_flightInfo">
								<div class="voyage">启程</div>
								<div class="ibWrap">
									<div class="">卡塔尔航空</div>
									<div class="">16h5m</div>
									<div class="">1次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>北京</span>01:25</div>
									<div class="airport"><span>多哈</span>05:35</div>
									<div class="airport"><span>多哈</span>06:45</div>
									<div class="airport"><span>布达佩斯</span>10:30</div>
								</div>
							</div>
							<div class="com_flightInfo">
								<div class="voyage">回程</div>
								<div class="ibWrap">
									<div class="">卡塔尔航空</div>
									<div class="">15h30m</div>
									<div class="">1次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>布拉格</span>16:10</div>
									<div class="airport"><span>多哈</span>00:10+1</div>
									<div class="airport"><span>多哈</span>02:05</div>
									<div class="airport"><span>北京</span>14:40</div>
								</div>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="departure">香港出发</div>
						<div class="autoWrapper">
							<div class="com_flightInfo">
								<div class="voyage">启程</div>
								<div class="ibWrap">
									<div class="">荷兰皇家</div>
									<div class="">16h30m</div>
									<div class="">1次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>香港</span>13:35</div>
									<div class="airport"><span>阿姆斯特丹</span>18:55</div>
									<div class="airport"><span>阿姆斯特丹</span>21:05</div>
									<div class="airport"><span>布达佩斯</span>23:05</div>
								</div>
							</div>
							<div class="com_flightInfo">
								<div class="voyage">回程</div>
								<div class="ibWrap">
									<div class="">荷兰皇家</div>
									<div class="">14h20m</div>
									<div class="">1次中转</div>
								</div>
								<div class="flex-row-start-between">
									<div class="airport"><span>布拉格</span>14:30</div>
									<div class="airport"><span>阿姆斯特丹</span>16:00</div>
									<div class="airport"><span>阿姆斯特丹</span>17:40</div>
									<div class="airport"><span>香港</span>11:50+1</div>
								</div>
							</div>
						</div>
					</div>

				</div>
				<!-- 如果需要分页器 -->
				<div class="swiper-pagination">
				</div>
			</div>
		</div>
		<!-- Reviews -->
		<div class="site reviews">
			<h4>来自客户的评价</h4>
			<div class="swiper-container section">
				<div class="swiper-wrapper">
					<div class="swiper-slide">
						<div class="com_dialog">
							<div class="dialogWrap">
								<div class="rate">
									<i class="fa fa-star active" aria-hidden="true"></i>
									<i class="fa fa-star active" aria-hidden="true"></i>
									<i class="fa fa-star active" aria-hidden="true"></i>
									<i class="fa fa-star active" aria-hidden="true"></i>
									<i class="fa fa-star" aria-hidden="true"></i>
								</div>
								<div class="dialog">
									第一次在国外过圣诞节和跨年，阿姆斯特丹真是个神奇的城市，既充满文艺气息，又有如此光怪陆离的一面；三个城市中间，个人更喜欢伦敦，以及伦敦人的伦敦腔，伦敦当地的local guide也超级可爱幽默，圣诞节当天还给我们准备了Mince Pie；有机会想再去待久一些，最想感谢一起旅行的小伙伴们，一起跨年的经历实在太难忘。
								</div>
								<div class="dialogBelongTo"></div>
							</div>
						</div>
						<div class="userInfo">
							<span class="name">Jax Oddo,</span>
							<span class="position">外籍导游</span>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiper-slide">
							<div class="com_dialog grayBlue">
								<div class="dialogWrap">
									<div class="rate">
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star" aria-hidden="true"></i>
									</div>
									<div class="dialog">
										第一次在国外过圣诞节和跨年，阿姆斯特丹真是个神奇的城市，既充满文艺气息，又有如此光怪陆离的一面；三个城市中间，个人更喜欢伦敦，以及伦敦人的伦敦腔，伦敦当地的local guide也超级可爱幽默，圣诞节当天还给我们准备了Mince Pie；有机会想再去待久一些，最想感谢一起旅行的小伙伴们，一起跨年的经历实在太难忘。
									</div>
									<div class="dialogBelongTo"></div>
								</div>
							</div>
							<div class="userInfo">
								<span class="name">Jax Oddo,</span>
								<span class="position">外籍导游</span>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiper-slide">
							<div class="com_dialog">
								<div class="dialogWrap">
									<div class="rate">
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star" aria-hidden="true"></i>
									</div>
									<div class="dialog">
										第一次在国外过圣诞节和跨年，阿姆斯特丹真是个神奇的城市，既充满文艺气息，又有如此光怪陆离的一面；三个城市中间，个人更喜欢伦敦，以及伦敦人的伦敦腔，伦敦当地的local guide也超级可爱幽默，圣诞节当天还给我们准备了Mince Pie；有机会想再去待久一些，最想感谢一起旅行的小伙伴们，一起跨年的经历实在太难忘。
									</div>
								</div>
								<div class="dialogBelongTo"></div>
							</div>
							<div class="userInfo">
								<span class="name">Jax Oddo,</span>
								<span class="position">外籍导游</span>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiper-slide">
							<div class="com_dialog grayBlue">
								<div class="dialogWrap">
									<div class="rate">
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star" aria-hidden="true"></i>
									</div>
									<div class="dialog">
										第一次在国外过圣诞节和跨年，阿姆斯特丹真是个神奇的城市，既充满文艺气息，又有如此光怪陆离的一面；三个城市中间，个人更喜欢伦敦，以及伦敦人的伦敦腔，伦敦当地的local guide也超级可爱幽默，圣诞节当天还给我们准备了Mince Pie；有机会想再去待久一些，最想感谢一起旅行的小伙伴们，一起跨年的经历实在太难忘。
									</div>
									<div class="dialogBelongTo"></div>
								</div>
							</div>
							<div class="userInfo">
								<span class="name">Jax Oddo,</span>
								<span class="position">外籍导游</span>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiper-slide">
							<div class="com_dialog">
								<div class="dialogWrap">
									<div class="rate">
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star" aria-hidden="true"></i>
									</div>
									<div class="dialog">
										第一次在国外过圣诞节和跨年，阿姆斯特丹真是个神奇的城市，既充满文艺气息，又有如此光怪陆离的一面；三个城市中间，个人更喜欢伦敦，以及伦敦人的伦敦腔，伦敦当地的local guide也超级可爱幽默，圣诞节当天还给我们准备了Mince Pie；有机会想再去待久一些，最想感谢一起旅行的小伙伴们，一起跨年的经历实在太难忘。
									</div>
									<div class="dialogBelongTo"></div>
								</div>
							</div>
							<div class="userInfo">
								<span class="name">Jax Oddo,</span>
								<span class="position">外籍导游</span>
							</div>
						</div>
					</div>
					<div class="swiper-slide">
						<div class="swiper-slide">
							<div class="com_dialog grayBlue">
								<div class="dialogWrap">
									<div class="rate">
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star active" aria-hidden="true"></i>
										<i class="fa fa-star" aria-hidden="true"></i>
									</div>
									<div class="dialog">
										第一次在国外过圣诞节和跨年，阿姆斯特丹真是个神奇的城市，既充满文艺气息，又有如此光怪陆离的一面；三个城市中间，个人更喜欢伦敦，以及伦敦人的伦敦腔，伦敦当地的local guide也超级可爱幽默，圣诞节当天还给我们准备了Mince Pie；有机会想再去待久一些，最想感谢一起旅行的小伙伴们，一起跨年的经历实在太难忘。
									</div>
								</div>
								<div class="dialogBelongTo"></div>
							</div>
							<div class="userInfo">
								<span class="name">Jax Oddo,</span>
								<span class="position">外籍导游</span>
							</div>
						</div>
					</div>
				</div>
				<!-- 如果需要分页器 -->
				<div class="swiper-pagination">
				</div>
			</div>
		</div>

		<!-- Get a personalized quotation -->
		<div class="section quoBg" name="getPersonalizedPrice" id="getPersonalizedPrice">

			<div class="quotation">

				<h4>获取实时价格</h4>

				<div class="quotationBg"></div>

				<div class="quotationWrap">

					<?php include_once('discount.php') ; echo $discount2 ;?>

					<?php include_once('quotation-form.php'); echo $text1 ;?>

				</div>

				<?php include_once('sales-wrap.php') ;?>

			</div>

		</div>
		<!-- More trips like this -->
		<div class="site">
			<!-- trips -->
			<?php include_once('recommend.php') ;?>
		</div>
		<!-- From the blog -->
		<div class="site blog">
			<h4 class="flex-row-end-between">
				博客
				<a href="http://travelplus.ef.com.cn/blog/" class="blueBorderBtn">全部</a>
			</h4>
		</div>
		<!-- footer -->
		<?php include_once('common-footer.php') ;?>
		<?php include_once('popup.php') ;?>
		<script src="js/jquery-2.1.1.min.js"></script>
		<script src="js/bootstrap.min.js"></script>
		<script src="js/sticky.min.js"></script>
		<script src="js/itinerary.js"></script>
		<script src="js/jquery.js"></script>
		
		<script src="js/jquery.jfeed.js"></script>
		<script src="js/readRss.js"></script>
		<script type="text/javascript">
		var itineraryName = "<?php echo $pageName;?>";
		var currCoutries = getCountries();
        for(var itemIndex in currCoutries)
        {
            var itemInArray = currCoutries[itemIndex];
            var result = itineraryName.match(itemInArray);
            if (result) {
                getRssFetch('http://travelplus.ef.com.cn/blog/?tag='+result+'&feed=rss2');
                break;
            }
            else {
                 getRssFetch('http://travelplus.ef.com.cn/blog/?tag=europe&feed=rss2');
                 break;
            }
        }
		</script>
	</body>

</html>